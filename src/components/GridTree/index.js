...
class Tree extends BaseComponent {
    ...
    getShowRows () {
        const items = [];
        this._getShowRows(this.state.data, items);
        return items;
    }

    _getShowRows (data, ret) {
        data.forEach((item) => {
            ret.push(item);
            if (item.open) {
                if (item.children && item.children.length) {
                    this._getShowRows(item.children, ret);
                }
            }
        });
    }
    ...
    updateScrollSize () {
        const rows = this.getShowRows();
        const h = rows.length * 40;
        this.setState({
            boxHeight: h
        }, () => {
    ...
    render () {
        const className = classNames('cm-grid-tree', this.props.className);
        const autoHeight = this.props.autoHeight;
        
        const tree = <div className='cm-tree'>
            <TreeSubNodes
                ...
                onOpenClose={(item) => {
                    ...
                    this._openClose.call(this, item);
                }}
        ...
        return (
            <div className={className} style={this.props.style}>
                <Head columns={this.props.columns}/>
                {
                    autoHeight
                        ? tree
                        : <div className='cm-grid-tree-scroll'>
                            <div className='cm-grid-tree-scroll-box' ref={(f) => this.box = f}>
                                <div className='cm-grid-tree-scroll-spacer' ref={(f) => this.spacer = f} style={{height: this.state.boxHeight}}></div>
                            </div>
                            <div className='cm-grid-tree-scroll-content' ref={(f) => this.content = f}>
                                <div className='cm-grid-tree-body-wrap' ref={(f) => this.bodyWrap = f}>
                                    <div className='cm-grid-tree-body' ref={(f) => this.body = f}>
                                        {tree}
                                    </div>
                                </div>
                            </div>
