...
function svgFromObject (obj) {
    ...
    for ( const field in obj) {
        if (Object.prototype.hasOwnProperty.call(obj, field) && field !== 'tagName') {
            if (field === 'children') {
...
class Animation extends React.Component {
    ...
    constructor (props) {
        ...
        // this._realtimeSVG = new SvgButton(, viewModel.playRealtimeViewModel);
    ...
    setShuttleRingFromMouseOrTouch (e) {
        const shuttleRingDragging = this.shuttleRingDragging;
        ...
        if (e.type === 'mousedown' || (shuttleRingDragging && e.type === 'mousemove')) {
            ...
            const pointerRect = this._shuttleRingPointer.getBoundingClientRect();
            ...
            const shuttleRingAngle = this.shuttleRingAngle;
            console.log(angle, shuttleRingAngle);
            if (shuttleRingDragging || (clientX < pointerRect.right && clientX > pointerRect.left && clientY > pointerRect.top && clientY < pointerRect.bottom)) {
                widgetForDrag = this;
                this.shuttleRingDragging = true;
                this.setState({angle});
                this.shuttleRingAngle = angle;
    ...
    slower () {
        const index = this.getTypicalMultiplierIndex(this.multiplier, this._allShuttleRingTicks) - 1;
        if (index >= 0) {
            const multiplier = this._allShuttleRingTicks[index];
            this.updateMultiplier(multiplier);
            const angle = this.multiplierToAngle(multiplier, this._allShuttleRingTicks);
            
            this.setState({
                angle
            });
            if (this.props.onMultiplierChange) {
                this.props.onMultiplierChange(multiplier, ClockStep.SYSTEM_CLOCK_MULTIPLIER);
            }
        }
    }
    ...
    faster () {
        const index = this.getTypicalMultiplierIndex(this.multiplier, this._allShuttleRingTicks) + 1;
        if (index < this._allShuttleRingTicks.length) {
            const multiplier = this._allShuttleRingTicks[index];
            this.updateMultiplier(multiplier);
            const angle = this.multiplierToAngle(multiplier, this._allShuttleRingTicks);
    ...
    getTypicalMultiplierIndex (multiplier, shuttleRingTicks) {
    ...
    setShuttleRingTicks () {
        ...
        const positiveTicks = this.defaultTicks;
        ...
        for (i = 0, len = positiveTicks.length; i < len; ++i) {
            ...
            // filter duplicates
            if (!Object.prototype.hasOwnProperty.call(hash, tick)) {
                hash[tick] = true;
    ...
    angleToMultiplier (angle, shuttleRingTicks) {
    ...
    multiplierToAngle (multiplier, shuttleRingTicks) {
        if (this.clock && this.clock.clockStep === ClockStep.SYSTEM_CLOCK) {
            this.shuttleRingAngle = realtimeShuttleRingAngle;
            return realtimeShuttleRingAngle;
        }

        if (Math.abs(multiplier) <= 1) {
            this.shuttleRingAngle = multiplier * realtimeShuttleRingAngle;
        ...
        if (multiplier > 0) {
            maxv = Math.log(shuttleRingTicks[shuttleRingTicks.length - 1]);
            scale = (maxv - minv) / (maxp - minp);
            const angle = (Math.log(multiplier) - minv) / scale + minp;
            this.shuttleRingAngle = angle;
            return angle;
        }
        ...
        const angle = -((Math.log(Math.abs(multiplier)) - minv) / scale + minp);
    ...
    setShuttleRingPointer () {
        if (this._shuttleRingPointer) {
            this._shuttleRingPointer.setAttribute('transform', `translate(100,100) rotate(${this.state.angle})`);

            const angle = Math.max(Math.min(this.state.angle, maxShuttleRingAngle), -maxShuttleRingAngle);
            if (!this._allShuttleRingTicks) {
                this.setShuttleRingTicks();
            }
            const ticks = this._allShuttleRingTicks;
            ...
            if (Math.abs(angle) === maxShuttleRingAngle) {
                const multiplier = angle > 0 ? ticks[ticks.length - 1] : ticks[0];
                if (this.props.onMultiplierChange) {
                    this.props.onMultiplierChange(multiplier, ClockStep.SYSTEM_CLOCK_MULTIPLIER);
                }
                this.updateMultiplier(multiplier);
            ...
            let multiplier = this.angleToMultiplier(angle, ticks);
            if (multiplier !== 0) {
    ...
    playRealtimeCommand = () => {
        const angle = this.multiplierToAngle(1.0);
        this.setState({
            isPaused: false,
            angle
        });
        ...
        if (this.props.onMultiplierChange) {
            this.props.onMultiplierChange(1.0, ClockStep.SYSTEM_CLOCK);
        }
    }
    ...
    playForwardCommand = () => {
        if (this.clock) {
            const multiplier = this.clock.multiplier;
            if (multiplier < 0) {
                this.clock.multiplier = -multiplier;
            }
            this.updateMultiplier(this.clock.multiplier);
            const angle = this.multiplierToAngle(this.clock.multiplier, this._allShuttleRingTicks);
            ...
            if (this.props.onMultiplierChange) {
                this.props.onMultiplierChange(this.clock.multiplier, ClockStep.SYSTEM_CLOCK_MULTIPLIER);
            }
    ...
    componentWillUnmount () {
        Events.off(this._realtimeSVG, 'click', this.playRealtimeCommand);
        Events.off(this._pauseSVG, 'click', this.pausedCommand);
        Events.off(this._playForwardSVG, 'click', this.playForwardCommand);
    }

    componentDidMount () {
        this._realtimeSVG = wingButton(3, 4, '#animation_pathClock');
        Events.on(this._realtimeSVG, 'click', this.playRealtimeCommand);
        this._playReverseSVG = rectButton(44, 99, '#animation_pathPlayReverse');
        this._playForwardSVG = rectButton(124, 99, '#animation_pathPlay');
        this._pauseSVG = rectButton(84, 99, '#animation_pathPause');
        Events.on(this._pauseSVG, 'click', this.pausedCommand);
        Events.on(this._playForwardSVG, 'click', this.playForwardCommand);
        Events.on(this._playReverseSVG, 'click', this.playReverseCommand);

        const buttonsG = document.createElementNS(svgNS, 'g');
        buttonsG.appendChild(this._realtimeSVG);
        buttonsG.appendChild(this._playReverseSVG);
        buttonsG.appendChild(this._playForwardSVG);
        buttonsG.appendChild(this._pauseSVG);
        ...
        this._topG.appendChild(shuttleRingBackG);
        this._topG.appendChild(knobG);
        this._topG.appendChild(buttonsG);
        ...
        const that = this;
        const mouseCallback = function (e) {
            that.setShuttleRingFromMouseOrTouch(e);
        };
    ...
    updateTime (time) {
        if (!this.state.isPaused) {
            ...
            timeNode.textContent = time.format('HH:mm:ss');
            dateNode.textContent = time.format('YYYY-MM-DD');
        }
    }
    ...
    updateMultiplier (multiplier) {
        ...
        const statusNode = this._knobStatus.childNodes[0];
        statusNode.textContent = `${multiplier}x`;
    }

    render () {
        this.setShuttleRingPointer();
        return (
            <div ref={(f) => this._container = f} className='cm-time-animation cesium-viewer-animationContainer'>
                <div ref={(f) => this._theme = f} className='cesium-animation-theme'>
                    <div ref={(f) => this._themeNormal = f} className='cesium-animation-themeNormal'></div>
                    <div ref={(f) => this._themeHover = f} className='cesium-animation-themeHover'></div>
                    <div ref={(f) => this._themeSelect = f} className='cesium-animation-themeSelect'></div>
                    <div ref={(f) => this._themeDisabled = f} className='cesium-animation-themeDisabled'></div>
                    <div ref={(f) => this._themeKnob = f} className='cesium-animation-themeKnob'></div>
                    <div ref={(f) => this._themePointer = f} className='cesium-animation-themePointer'></div>
                    <div ref={(f) => this._themeSwoosh = f} className='cesium-animation-themeSwoosh'></div>
                    <div ref={(f) => this._themeSwooshHover = f} className='cesium-animation-themeSwooshHover'></div>
                </div>

                <svg ref={(f) => this._svgNode = f} xmlns='http://www.w3.org/2000/svg'>
                    <g ref={(f) => this._topG = f}></g>
                </svg>
            </div>
        );
    }
