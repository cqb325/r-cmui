...
class Progress extends BaseComponent {
    ...
    render(){
        ...
        let current = parseInt((this.state.value - this.state.min) / (this.state.max - this.state.min) * 100);
        let width = current+"%";
        let percent = showPercent ? width : null;

        if(status === "finished" && showPercent){
            let icon = type === "circle" ? "check" : "check-circle";
            percent = <FontIcon icon={icon}></FontIcon>;
        }
        ...
        let bar = null, fontSize = 25;
        if(type === "circle"){
            bar = this.renderCircle();
            fontSize = fontSize * this.props.radius / 60;
        }else{
            bar = <div className="cm-progress-bar" style={{width: width, height: this.props.strokeWidth}}></div>;
        }
        return (
            <div className={className} style={style}>
                <div className="cm-progress-outer">
                    <div className="cm-progress-inner">
                        {bar}
                        {type === "circle" ? <span className="cm-progress-info" style={{fontSize: fontSize}}>{percent}</span>:null}
                    </div>
                </div>
